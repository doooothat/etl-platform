...
## 🕒 마지막 업데이트
- **일시**: 2026-04-03T23:59:00+09:00
- **에이전트**: Antigravity (Google Deepmind)
- **상태**: 시스템 전체 정지(stop) 테스트 완료 및 강제 종료 로직 완벽 고도화 (현재 상태: 모든 파드 스케일 0)
...
- **에이전트 좀비 현상 원천 차단 (Airflow Redis 이슈)**:
  - 기존의 `kubectl delete pod --force` 구문이 쿠버네티스에는 파드를 즉시 없애지만, 컨테이너 런타임(OrbStack/Docker) 단에는 Orphan(좀비) 프로세스를 남긴 채로 계속 돌아가는 현상을 확인.
  - 이를 해결하기 위해 `manage-project.sh`의 `stop` 로직에 `kubectl delete --force`와 함께 **`docker ps`로 조회 후 시스템 도커(`docker rm -f`) 레벨에서 직접 물리적으로 컨테이너를 박멸하는 레이저 스크립트**를 공식적으로 편입함.
- **프로메테우스 오퍼레이터 부활(Reconcile) 버그 해결**:
  - `stop` 실행 시 프로메테우스 오퍼레이터가 죽기 직전 프로메테우스 서버 파드를 1개로 다시 살려내고 죽는 이슈 발견.
  - 스크립트 실행 극초반에 아예 K8s CRD(`prometheus.monitoring`)의 `spec.replicas`를 `0`으로 영구 패치(`patch`) 한 뒤에 셧다운하도록 스크립트를 개선하여 좀비 부활 원천 차단 완료.
...
### 실행 중인 서비스 (All Stopped - Scale: 0)
현재 완벽한 셧다운 테스트를 위해 **`./manage-project.sh stop` 명령어가 실행된 상태**입니다. 모든 네임스페이스의 워크로드가 스케일 0으로 내려갔으며, Persistent Volume(PVC/PV) 또한 완전히 초기화되었고 Orbstack의 좀비 컨테이너 및 프로메테우스 부활 파드까지 깨끗하게 제거되어 안심하고 다음 작업을 진행(또는 `start`)할 수 있습니다.
